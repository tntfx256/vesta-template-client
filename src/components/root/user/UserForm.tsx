...
export const UserForm: ComponentType<IUserFormProps> = (props: IUserFormProps) => {
  ...
  const service = getCrudInstance<IUser>("user");
  ...
  useEffect(() => {
    ...
    service.fetch(id).then(data => {
      if (data.image) {
        data.image = getFileUrl(`user/${data.image}`);
      }
      setUser(data);
    });
  ...
  return (
    <FormWrapper name="userForm" onSubmit={onSubmit}>
      <Select name="type" label={tr("fld_type")} value={user.type} error={errors.type} onChange={onChange} options={typeOptions} />
      <Select name="role" label={tr("role")} options={roles} value={roleId} error={errors.role} onChange={onChange} titleKey="name" valueKey="id" />
      <TextInput name="username" label={tr("fld_username")} value={user.username} error={errors.username} onChange={onChange} />
      ...
      <TextInput name="email" label={tr("fld_email")} value={user.email} error={errors.email} onChange={onChange} type="email" />
      <TextInput name="mobile" label={tr("fld_mobile")} value={user.mobile} error={errors.mobile} onChange={onChange} />
