...
export const RoleForm: ComponentType<IRoleFormProps> = (props: IRoleFormProps) => {
  ...
  const roleService = getCrudInstance<IRole>("acl/role");
  ...
  const [role, setRole] = useState<IRole>({ name: "", permissions: [], status: Status.Inactive });
  ...
  useEffect(() => {
    let rawPermissions = [];
    getCrudInstance<IPermission>("acl/permission")
      .fetchAll({})
      .then(items => {
  ...
  return (
    <FormWrapper name="roleAddForm" onSubmit={onSubmit}>
      <div className="roleForm-component">
        <TextInput name="name" label={tr("fld_name")} value={role.name} error={errors.name} onChange={onChange} />
        <TextInput name="desc" label={tr("fld_desc")} value={role.desc} error={errors.desc} onChange={onChange} />
        <Select name="status" label={tr("fld_status")} value={role.status} titleKey="title" error={errors.status} onChange={onChange} options={statusOptions} />
        <div className="form-group">
          <label>{tr("fld_permission")}</label>
          <ul className="permission-list">{rows}</ul>
        </div>
        {props.children}
      </div>
    </FormWrapper>
  ...
  function parsePermissions(rawPermissions: IPermission[], rawRole: IRole) {
    ...
    for (let i = 0, il = rawPermissions.length; i < il; ++i) {
      ...
      permissions[p.resource].push({ id: p.id, action: p.action, resource: "" });
    }
  ...
  function renderPermissionTable() {
    ...
    for (let i = resources.length; i--; ) {
      ...
      list.push(
        <li key={i}>
          <Multichoice name={resource} label={resource} value={values} onChange={onPermissionChange} options={actions} titleKey="action" />
        </li>
      );
    }
    return list;
